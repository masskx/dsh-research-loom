...
export function OverviewPanel({ project, report, workflow, nextStage, onStage, saveConfig, checkResults, language }) {
  ...
  return h('div', { style: { display: 'grid', gap: 14 }, 'data-testid': 'workbench-overview' },
    h('details', { style: box }, h('summary', { style: { cursor: 'pointer' } }, en ? 'Full workflow recommendation' : '完整流程建议（可选）'),
    ...
    project.tasks.length ? h('section', { style: box },
      ...
      h('small', null, en ? 'New paths are candidates, not proof of completion or authorship. Accepting a task does not confirm a whole stage.' : '新增路径是候选产物，不代表任务已完成或由该任务生成；验收任务不会自动确认整个阶段。')) : null,
...
export function ProjectTransfer({ project, saveConfig, language }) {
  ...
  return h('details', { style: box }, h('summary', { style: { cursor: 'pointer' } }, en ? 'Export / import project memory' : '导出 / 导入项目记忆'),
    ...
    pending ? h('div', { style: box },
      ...
      h('button', { style: button, onClick: async () => { if (await saveConfig({ ...pending, tasks: project.tasks, reviewLoop: project.reviewLoop }) !== false) { setPending(null); setMessage(en ? 'Imported.' : '已导入。'); } } }, en ? 'Apply snapshot' : '应用此快照'),
