...
当前版本 **0.9.3** · 已验证宿主 **DSH 0.1.1-rc.2** · **CC-BY-NC-4.0（非商业）**

0.9.3 修复含大量源码的项目中，主稿和审稿意见被挤出新手任务候选路径的问题。候选材料按任务类型选取并说明抽样边界，仍需作者确定主稿和审稿轮次。保留 0.9.2 的稿件身份、异步任务和扫描修复；3 个缺少上游执行程序的命令仍未启用。升级后需重启 DSH 并刷新浏览器，详见 [更新说明](docs/CHANGELOG.zh-CN.md)。
