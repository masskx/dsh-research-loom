...
当前版本 **0.10.0** · 已验证宿主 **DSH 0.1.1-rc.2** · **CC-BY-NC-4.0（非商业）**

0.10.0 将新手返修接成连续任务：确认主稿和原始意见文件，直接整理有来源的任务卡，勾选文字修改，查看真实文本差异，并在中断后只读恢复。历史意见或版本不清楚可以明确标记，不需要先手写复杂提示词。见 [第一次返修指南](docs/NOVICE-REVISION.zh-CN.md)；升级后重启 DSH 并刷新浏览器。
...
“准备任务到对话框”会填入可编辑提示词，**不会自动发送**。返修入口确认材料后，“读取原始意见并建立返修清单”会直接发送一次模型任务，结果显示在返修任务卡及主对话中。
...
1. 将主稿、编辑决定信和原始审稿意见放入工作区，选择 **开始 → 论文返修 → 梳理审稿意见**。从候选中选择文件，或填工作区相对路径。
2. 选择意见与稿件的关系：对应当前稿、历史意见或不知道。点击 **确认本轮材料 → 读取原始意见并建立返修清单**；此步只分析，不改稿。
3. 在任务卡查看原文出处、通俗解释、缺少的证据和完成条件，勾选本轮文字任务，再点击 **授权本轮修改并执行**。实验与作者决策项保留待办。
4. 查看独立修订稿及 **实际文件差异**，导出返修检查清单与回复填写模板。中断时指定已生成的新稿，启动一次只读恢复。最终回复与投稿由作者核验。
...
3. 查看问题、涉及章节和待补证据，勾选本轮要处理的文字任务；熟悉流程时也可展开自定义范围。点击 **授权本轮修改并执行**。
...
修改与复核要求独立新稿、保留全部原意见及编号；结果格式不完整、遗漏问题或缺证据会停止。
0.9.2 会在接受结果及后续执行前，通过宿主核对原稿与修订稿是否真实存在于当前工作区；不同路径写法或硬链接指向同一文件时，不会被当作独立新稿。用户明确指定的原稿必须匹配。
模型提供的修改证据仍须作者核验。0.10.0 可由宿主只读比较两份 UTF-8 文本稿件并显示变化覆盖区域及 SHA-256；不调用模型。每份最多 512 KiB、每侧预览最多 8,000 字符，截断会明确提示。Word/PDF、LaTeX 子文件及排版仍需外部工具；当前指纹不能证明原稿从未被修改。
...
已有论文和从零开始的通用引导任务只填入对话框，手动发送后检查结果；返修材料确认入口则直接发送并进入返修记录。
...
- 自动盘点实时读取当前会话工作区的文件路径，先应用子目录与排除规则，再按上限扫描，最多返回 2,000 项；引导提示最多列出 60 项，不保证穷尽所有文件。可手动纠正归类。扫描失败会保留同工作区的上次清单，并标注失败状态。
...
- 上游 Python 核验脚本、shared 契约及 Claude Code hooks 未移植。`/ars-mark-read`、`/ars-unmark-read`、`/ars-cache-invalidate` 暂不注册；其余技能加载时明确检查宿主能力，不将缺失的确定性检查描述为已执行。多角色描述不代表宿主一定并行启动多个代理。
...
保留 4 个核心技能与 16 份命令源文件；当前注册其中 13 个 `/ars-*` 命令，3 个依赖未移植执行程序的命令暂不启用。增加交互工作台、材料归类、任务回流、项目记忆与新手引导。
