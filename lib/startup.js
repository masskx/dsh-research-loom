...
//   - 13 available user-invocable commands; 3 upstream script-only wrappers
//     remain bundled but are not registered without their unported runtime.
//
// Also registers a read-only, session-scoped material scanner. Schema
// dependencies are declared in package.json and installed with the plugin.
//
...
import { registerMaterialScanner } from './material-scan.js';
import { DSH_COMPATIBILITY, UNAVAILABLE_COMMANDS } from './skill-compatibility.js';
...
const ProjectState = z.object({
  ...
  revisionIntake: z.object({
    manuscript: z.string().default(''), reviewFiles: z.array(z.string()).default([]),
    round: z.string().default(''), relationship: z.union(['unknown', 'current', 'historical']).default('unknown'),
    confirmed: z.boolean().default(false),
  }).default({ manuscript: '', reviewFiles: [], round: '', relationship: 'unknown', confirmed: false }),
...
function loadSkill(rel) {
  ...
  return {
    name,
    description,
    content: DSH_COMPATIBILITY + content,
...
export default function apply(ctx) {
  ...
  if (existsSync(commandsDir)) {
    for (const entry of readdirSync(commandsDir)) {
      ...
      if (!skill) continue;
      if (Object.hasOwn(UNAVAILABLE_COMMANDS, skill.name)) continue;
      definitions.push({
        ...skill,
        source: 'runtime',
  ...
  registerMaterialScanner(ctx, settings);
  let registrations = [];
