...
function buildEntryPrompt(state, report, { language = "zh", cwd = "", currentDate = (/* @__PURE__ */ new Date()).toISOString() } = {}) {
  ...
  return [
    ...
    en ? `Showing ${sourceFiles.length} of ${report.sourceFiles.length} scanned candidate paths, prioritized by task and material type. This sample is not the full inventory and does not identify the authoritative manuscript or review round. Resolve version ambiguity before editing.` : `按任务和材料类型优先展示已扫描的 ${report.sourceFiles.length} 个候选中的 ${sourceFiles.length} 个路径。这是导航抽样，不是完整目录，也不代表已确定主稿或审稿轮次；修改前必须核对版本歧义。`,
...
async function validateLoopLaunch(base, phase, validateFiles) {
  ...
  const checked = await validateLoopFiles(
    ...
    { ...base, phase: checkedPhase, manuscript },
    validateFiles
  );
...
function ReviewLoopPanel({ project, conversation, input, inputActions, cwd, sessionId, saveConfig, validateFiles, writable, enabled }) {
  ...
  const [observationTick, setObservationTick] = React3.useState(0);
  const lock = React3.useRef(false);
  const observationPending = React3.useRef(false);
...
var scanRemote = { package: "dsh-academic-research-skills", descriptors: [descriptor, reviewDescriptor] };
...
function PaperStatusDock({ ctx, scope, sessionId, useSessions, useSession, useInput, inputActions, session, heroOnly = false }) {
  ...
  const validateFiles = React4.useCallback(async (request) => {
...
[Research Loom task ${task.id}]`);
  ...
  return React4.createElement(
    ...
    React4.createElement("div", { hidden: view !== "overview" }, React4.createElement(ReviewLoopPanel, { key: `loop-${cwd}-${sessionId}`, project, conversation, input, inputActions, cwd, sessionId, saveConfig, validateFiles, writable: snapshot.writable, enabled })),
